...
class VGG19(object):
    ...
    def build_model(self): #建立网络结构
        ...
        for layer_name in self.layers.keys(): #仅卷积层和全连接层权重矩阵要更新
    ...
    def load_image(self, image_dir):
        ...
        self.input_image = Image.fromarray(self.input_image).resize([224,224])
        ...
        # input dim [N, height, width, channel]
        # TODO：调整图片维度顺序以适应标准模型配置
...
if __name__ == '__main__':
    ...
    vgg.init_model()
    vgg.load_model()
    vgg.load_image('./picture/cat1.jpg')
    prob = vgg.evaluate()

...
class VGG19(object):
    ...
    def load_image(self, image_dir, image_height, image_width):
        ...
        self.input_image = plt.imread(image_dir)
        image_shape = self.input_image.shape
        self.input_image = Image.fromarray(self.input_image).resize([image_height,image_width])
    ...
    def save_image(self, input_image, image_shape, image_dir):
        ...
        input_image = input_image[0] + self.image_mean #[height,width,channel]
        input_image = np.clip(input_image, 0, 255).astype(np.uint8)
        input_image = Image.fromarray(input_image).resize([image_shape[0],image_shape[1]])
        plt.imsave(image_dir, input_image)
    ...
    def forward(self, input_image, layer_list):
        ...
        for idx in range(len(self.param_layer_name)):
            # TODO： 计算VGG19网络的前向传播
            current = self.layers[self.param_layer_name[idx]].forward(current) #某一层的输出，是个张量
            if self.param_layer_name[idx] in layer_list:#需要计算损失的层
...
class AdamOptimizer(object):
    ...
    def update(self, input, grad):
        ...
        output = input - self.lr * mt_hat / (vt_hat ** 0.5 + self.eps)
        return output
...
if __name__ == '__main__':
    ...
    content_image, content_shape = vgg.load_image('./picture/weinisi.jpg', IMAGE_HEIGHT, IMAGE_WIDTH)
    style_image, _ = vgg.load_image('./picture/style.jpg', IMAGE_HEIGHT, IMAGE_WIDTH)
    #content_layers = vgg.forward(content_image, CONTENT_LOSS_LAYERS)
    #f = open('content.txt','wb')
    #pickle.dump(content_layers,f)
    #f.close()
    #style_layers = vgg.forward(style_image, STYLE_LOSS_LAYERS)
    #f = open('style.txt','wb')
    #pickle.dump(style_layers,f)
    #f.close()
    transfer_image = get_random_img(content_image, NOISE)

    content_layers = pickle.load(open('content.txt','rb'))
    style_layers = pickle.load(open('style.txt','rb'))
    vgg = pickle.load(open('vgg_6.pkl','rb'))
    for step in range(7,TRAIN_STEP):
        ...
        for layer in CONTENT_LOSS_LAYERS:
            # TODO： 计算内容损失的前向传播
            print ("Forwarding content "+layer)
            ...
            # TODO： 计算内容损失的反向传播
            print ("Backwarding content "+layer)
        ...
        f = open('vgg_'+str(step)+'.pkl','wb')
        pickle.dump(vgg,f)
        f.close()
        # TODO： 利用Adam优化器对风格迁移图像进行更新
        #print "Updating transfer image..."
